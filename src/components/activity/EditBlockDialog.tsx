...
import { type Tag, type TimeBlockWithTag } from '@/lib/types';
...
export function EditBlockDialog({
  ...
  const options =
    block.tag && !tags.some((t) => t.id === block.tag!.id)
      ? [
          ...tags,
          { ...block.tag, user_id: block.user_id, is_archived: true, created_at: '' } as Tag,
        ]
      : tags;
  ...
  return (
    <Dialog labelledBy="editBlockTitle" onDismiss={onCancel} busy={saving}>
      ...
      {block.paused_seconds > 0 && (
        <p className="text-muted small mb-3">
          {formatTotal(block.paused_seconds)} of this was paused and doesn&apos;t count towards the
          total.
        </p>
      )}
      ...
      <div className="mb-3">
        ...
        <TagPicker tags={options} value={tagId} onChange={setTagId}></TagPicker>
