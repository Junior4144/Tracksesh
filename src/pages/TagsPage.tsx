...
export default function TagsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchTags({ includeArchived: true }), fetchTagUsage()])
      .then(([rows, counts]) => {
        if (cancelled) return;
        setTags(rows);
        setUsage(counts);
  ...
  async function save(tag: Tag, patch: { name: string; color: string }) {
    ...
    setError(null);
    try {
      replace(await updateTag(tag.id, patch));
      setEditingId(null);
    } catch (e) {
  ...
  async function setArchived(tag: Tag, is_archived: boolean) {
    ...
    setError(null);
    try {
      replace(await updateTag(tag.id, { is_archived }));
    } catch (e) {
...
/**
 * The API answers a refusal with a sentence — it translates the constraint
 * names itself (see PostgresExceptionHandler.cs), so a name clash arrives here
 * already worded for a human rather than as the unique index's name. The
 * fallback covers the case where nothing reached the server at all.
 */
